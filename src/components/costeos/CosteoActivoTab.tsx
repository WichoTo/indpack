...
interface CosteoActivoTabProps {
  costeos: Costeo[];
  empresas: Empresa[];
...
const CosteoActivoTab: React.FC<CosteoActivoTabProps> = ({ costeos, empresas, onEdit, onDelete }) => {
  const activos = costeos.filter(c => c.estatus !== 'Entregado' && c.estatus !== 'Cancelado');
          ...
          {activos.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} align="center" sx={{ py: 4 }}>
                <Typography color="text.secondary">No hay costeos activos registrados.</Typography>
              </TableCell>
            </TableRow>
          ) : (
            activos.map(c => {
...
export default CosteoActivoTab;
